...
<div class="row">
    <div class="col-sm-4">
        <form method="post" action="aksi.php?act=rel_alternatif_ubah&ID=<?=$row->kode_siswa?>">
        <?php
        $rows = $db->get_results("SELECT ra.ID, k.kode_kriteria, k.nama_kriteria, ra.nilai FROM tb_rel_alternatif ra INNER JOIN tb_kriteria k ON k.id_kriteria=ra.id_kriteria  WHERE id_siswa='$_GET[ID]' ORDER BY kode_kriteria");
        foreach($rows as $row):?>
